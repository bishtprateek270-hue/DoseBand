...
def decode_qr_from_image_bytes(image_bytes: bytes) -> Tuple[Optional[str], Optional[str], Optional[str]]:
    ...
    try:
        ...
        # Attempt to parse as official DoseBand JSON payload
        # Strictly require 'app': 'DoseBand' to prevent scanning foreign/unrelated QR codes
        try:
            parsed = json.loads(raw_payload)
            if isinstance(parsed, dict) and parsed.get("app") == "DoseBand":
                w_id = parsed.get("worker_id")
                b_id = parsed.get("badge_id")
                if w_id and b_id:
                    return str(w_id).strip(), str(b_id).strip(), raw_payload
        except Exception:
            pass

        # Foreign or non-DoseBand QR code detected -> strictly reject
...
def validate_badge_profile(
    ...
    Strictly rejects foreign/third-party QR codes.
    ...
    """
    if not worker_id or not badge_id:
        return {
            "valid": False,
            "status": "UNRECOGNIZED",
            "worker": None,
            "message": "Unrecognized QR Code: Only official DoseBand worker badges generated by this platform are accepted. External URLs, barcodes, and foreign QR codes are strictly rejected."
        }
